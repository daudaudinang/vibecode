...
- **Walkthrough files**: `.claude/plans/<normalized-plan-name>/phase-N-walkthrough.md`
...
> 2. TRONG khối này, hãy phân tích luồng xử lý và định hình các lệnh Tool cần gọi (vd: `Read`, `Edit`, `Bash`).
...
3. Xác định:
   - **Tier của plan** (S/M/L)
   - **Context Sharding (Tier L)**: Nếu plan là Tier L và có chia nhỏ thành các Story files, ghi chú lại danh sách các story files tương ứng. Trong quá trình chạy thay vì nạp toàn bộ một lúc vào context, hãy đọc story file tương ứng khi vào mỗi phase.
   - Các phases, tasks.
...
1. Tạo output folder `.claude/plans/<normalized-name>/`.
...
1. Nếu là Tier L, đọc Story file tương ứng của phase này.
...
Tạo file walkthrough tại `.claude/plans/<normalized-name>/phase-{N}-walkthrough.md` với format:
...
📁 Walkthrough files:
- .claude/plans/<name>/phase-1-walkthrough.md
...
