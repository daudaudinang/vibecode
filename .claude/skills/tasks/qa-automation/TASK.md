...
> Command gốc vẫn có thể là `/qa-automation`, nhưng trong LP docs/orchestration nên ưu tiên `/lp:qa-automation`.

## Output

- QA report với evidence từ snapshots, console logs, network responses
- Verdict PASS/FAIL cho từng AC
- Human report: `.claude/pipeline/<PLAN_NAME>/05-qa-automation.output.md`
- Machine contract: `.claude/pipeline/<PLAN_NAME>/05-qa-automation.output.contract.json`
...
- user chỉ đưa ticket/task → đọc `.claude/plans/` hoặc task artifacts để tìm AC
...
- xoá file test tạm trong `.claude/tmp/` nếu có
