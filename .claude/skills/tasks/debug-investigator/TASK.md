...
1. Đọc mô tả lỗi từ user (qua `/lp:debug-investigator <mô tả>` hoặc yêu cầu tương đương).
...
4. Verify repo đã usable cho query/context/impact. Nếu verify pass → ghi nhận `HAS_GITNEXUS = true`.
5. Chỉ khi bootstrap/verify fail, hoặc GitNexus không đủ thông tin cho bug hiện tại, mới ghi nhận `HAS_GITNEXUS = false` và chuyển sang fallback `Grep` / `Read`. Khi đó phải nêu rõ degraded mode và lý do fallback.
6. Không được bỏ qua GitNexus chỉ vì bug có vẻ nhỏ.
...
### 1.5.3. Hành động agent ĐƯỢC PHÉP tự chạy

Agent được tự chạy các hành động sau để thu thập thêm thông tin:
...
- ✅ GitNexus readiness/bootstrap commands mức thấp như `gitnexus status`, `gitnexus analyze`, hoặc wrapper tương đương của runtime hiện tại — chỉ để đưa repo về trạng thái queryable cho lane debug

Agent **KHÔNG ĐƯỢC** tự chạy:
- 🚫 `python`, `pytest`, `uv run` cho reproduce/test app logic — risk chạy sai venv/môi trường
- 🚫 `npm run`, `bun run`, build commands của app — có thể thay đổi state hoặc tạo side effects ngoài nhu cầu debug read-only
...
> **VERIFICATION GATE (CHỐNG LỪA DỐI):** Bạn KHÔNG ĐƯỢC PHÉP bịa ra rủi ro dựa trên trí nhớ. Rủi ro chỉ được công nhận khi nó LÀ KẾT QUẢ TỪ VIỆC GỌI TOOL đọc code (`Read`, GitNexus context/query tools của runtime hiện tại, ...). Nếu chưa gọi Tool, mọi giả thuyết đều vô nghĩa.
...
> 1. **Fix phức tạp (≥4 files):** Tạo plan từ debug report → ưu tiên chuyển sang `/lp:plan` với debug report làm input. Các constraint read-only của debug task chỉ hết hiệu lực khi đã chuyển sang workflow planning/implement phù hợp.
...
> 3. **Kết thúc debug task** → chuyển sang workflow implement phù hợp, dùng kết quả debug làm input.
...
**Input:**
```
/lp:debug-investigator Login page hiện "Loading..." mãi không vào được dashboard
```
...
**Input:**
```
/lp:debug-investigator Giỏ hàng thỉnh thoảng hiện sai số lượng sau khi thêm sản phẩm,
...
- 🚫 **KHÔNG SỬA CODE** — Chỉ phân tích và đề xuất. Nếu user muốn fix → kết thúc debug task, chuyển sang `/lp:plan` hoặc workflow implement phù hợp.
...
- ✅ Với GitNexus bootstrap/readiness commands: dùng command canonical đã được repo verify (`npx gitnexus ...` hoặc wrapper tương đương của runtime hiện tại).
...
- Debug xong, user muốn fix → kết thúc debug task, ưu tiên gợi ý `/lp:plan` hoặc workflow implement phù hợp theo scope
...
3. Không được tự bịa report, confidence score, hay code snippet. Mọi evidence trong báo cáo phải khớp 100% với tool output thực tế.
