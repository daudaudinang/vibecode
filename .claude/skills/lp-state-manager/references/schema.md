...
- Default DB path được resolve thành `<repo-root>/.claude/state/pipeline_state.db`, không neo theo vị trí script.
...
- Artifact path chỉ được nằm dưới một trong các root:
  ...
  - `.claude/pipeline/`
  - `.claude/state/`
- `plan_path` canonical: `.claude/plans/PLAN_<NAME>/plan.md`
