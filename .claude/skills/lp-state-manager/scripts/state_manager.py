...
SCHEMA_VERSION = 2
V2_ALLOWED_ARTIFACT_PREFIXES = ('.claude/plans/', '.claude/pipeline/', '.claude/state/')
V2_ALLOWED_PLAN_PREFIX = '.claude/plans/'
...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        SELECT
          ...
          CASE
            ...
            ELSE '.claude/plans/' || plan_name || '/plan.md'
          END,
    ...
    conn.execute(
        ...
        SET artifact_path = CASE
          WHEN instr(artifact_path, '.claude/pipeline/') = 1 THEN artifact_path
          WHEN instr(artifact_path, '.claude/plans/') = 1 THEN artifact_path
          ELSE artifact_path
        END
        '''
    )
    conn.execute(
        ...
        UPDATE v2_plans
        SET plan_path = ?
        WHERE plan_path NOT LIKE '.claude/plans/%'
        ''',
        ('.claude/plans/UNKNOWN/plan.md',),
    )
    conn.execute(
        '''
        UPDATE v2_plans
        SET plan_path = '.claude/plans/' || plan_name || '/plan.md'
        WHERE plan_path = '.claude/plans/UNKNOWN/plan.md'
        ''',
    )
    conn.execute(
        '''
...
def create_workflow(conn: sqlite3.Connection, args: argparse.Namespace) -> dict[str, Any]:
    ...
    conn.execute(
        ...
        (
            args.plan_name,
            args.plan_name,
            normalize_plan_path(f'.claude/plans/{args.plan_name}/plan.md'),
