...
`.claude/active-plan` được phép tồn tại như pointer, nhưng phải trỏ về `.claude/plans/PLAN_<NAME>/`.
...
- `plan` = logical work package + canonical plan artifact package dưới `.claude/plans/PLAN_<NAME>/`
...
- Default state DB là `<repo-root>/.claude/state/pipeline_state.db`
- Nếu chạy nhiều LP workflows song song trong cùng workspace, `.claude/state/` và `.claude/pipeline/` là shared resources nên cần tránh reuse cùng `plan_name`
- Các worktree khác nhau tự nhiên được isolate vì mỗi worktree có `.claude/` riêng

## Agent execution policy

- Top-level LP worker steps (`create-plan`, `review-plan`, `implement-plan`, `review-implement`, `qa-automation`, `debug-investigator`) bắt buộc dùng agents; main chat là orchestrator, không tự thay worker step khi flow canonical yêu cầu agent worker.
...
- **Cấm** `run_in_background=true` cho top-level LP agents vì main chat cần result ngay để sync contract, set gate, và quyết định step kế tiếp.
...
1. Main chat nhận LP command canonical và giữ vai trò orchestrator
...
- Sai: bỏ agent và để main chat tự làm hết worker step
