...
- Canonical command catalog: `skill `lp-pipeline-orchestrator` (command index)`
- Repo-local guidance: `README.md`, `AGENTS.md`, `AGENTS.md rules section*`
...
- cấu trúc repo
- command catalog
- skills/rules trong `.codex/`
- config files thực tế (`package.json`, `pyproject.toml`, `tsconfig.json`, v.v.) nếu có
- canonical runtime manifest `.codex/config.toml` nếu repo đã khai báo
...
> Muốn check global install → dùng `python ~/.codex/scripts/vibecode_doctor.py`.
...
2. **Nếu repo không dùng `AGENTS.md` và user không yêu cầu tạo file**
   - chỉ trả summary hoặc đề xuất source-of-truth hiện có dưới `.codex/`
...
- `.agents/skills/lp:pipeline.md`
- `.agents/skills/**/SKILL.md` (chỉ các skill liên quan)
- `AGENTS.md rules section*.md`
...
- không ghi đè source-of-truth của `.codex/`
...
- Tech stack thực tế
- Source-of-truth hierarchy
- Skills overview từ `.agents/skills/*/SKILL.md`
- Rules overview từ `AGENTS.md rules section*.md`
