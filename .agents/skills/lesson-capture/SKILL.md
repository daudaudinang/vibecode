...
Thu thập bài học có giá trị từ task/session vừa hoàn thành, QUA 4 cổng kiểm chứng chất lượng, rồi ghi vào project `AGENTS.md` tại section lessons/patterns phù hợp — đảm bảo chỉ kiến thức đã verify, có evidence, và được user approve mới được lưu trữ.
...
→ Ghi vào: project `AGENTS.md` → section lessons/patterns/gotchas phù hợp
...
| User nói | Hành động |
|----------|----------|
| "Approve" / "OK" / "Đồng ý" | Ghi TẤT CẢ lessons vào project `AGENTS.md` |
| "Approve 1, reject 2" | Chỉ ghi lesson được approve |
| "Edit 1: sửa thành ..." | Sửa nội dung rồi ghi |
| "Skip" / "Bỏ qua" | KHÔNG ghi gì, kết thúc |

## Bước 5: Ghi vào project `AGENTS.md`

Sau khi user approve, append vào section lessons/patterns/gotchas tương ứng trong project `AGENTS.md`:
...
5. Dùng tool đọc file của runtime hiện tại (`Read` hoặc tương đương) đọc project `AGENTS.md` trước khi ghi → xác định vị trí chính xác
...
→ Ghi vào: project `AGENTS.md` → section gotchas tương ứng
...
→ Ghi vào: project `AGENTS.md` → section patterns/decisions tương ứng
...
**Agent ghi vào project `AGENTS.md`:**
...
- `.agents/skills/close-task/SKILL.md` — Gọi lesson-capture ở Bước 5
- Project `AGENTS.md` — nơi lưu project context/rules duy nhất của repo sau khi đã đồng nhất
- `.agents/skills/close-task/SKILL.md` hoặc task caller phải tôn trọng policy ghi vào project `AGENTS.md`
