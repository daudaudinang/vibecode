...
description: |
  ...
  chuẩn hóa context, rồi handoff sang LP canonical workflows hoặc tạo Jira issue mới khi user yêu cầu.
  Note: Codex không hỗ trợ slash commands; `/lp:*` trong file này là trigger-description patterns,
  không phải literal CLI commands.
---
...
- LP canonical orchestration: `.agents/skills/lp-pipeline-orchestrator/SKILL.md`
- LP command catalog: `skill `lp-pipeline-orchestrator` (command index)`
- LP runtime/state: `.agents/skills/lp-state-manager/SKILL.md`
...
> **Codex note:** `/lp:plan`, `/lp:implement`, `/lp:debug-investigator` ở bảng dưới là
> **trigger description patterns** cho skill invocation, không phải slash commands.
> Codex invoke chúng bằng description match hoặc orchestrator spawn.
...
- Plan package: `.codex/plans/PLAN_<NAME>/plan.md`
- Published pipeline artifacts: `.codex/pipeline/PLAN_<NAME>/NN-step.output.*`
...
- Với implementation: resolve plan file canonical tại `.codex/plans/PLAN_<NAME>/plan.md` trước khi handoff sang `/lp:implement`
...
- `.codex/pipeline/PLAN_<NAME>/04-review-implement.output.md`
...
Các resource này phải bám canonical `/lp:*` commands và `.codex/` artifact layout.
